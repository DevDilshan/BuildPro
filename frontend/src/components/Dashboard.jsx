"use client"
import { Building2, Users, DollarSign, AlertTriangle, TrendingUp, Clock, CheckCircle2, XCircle } from "lucide-react"
...
const Dashboard = () => {
  ...
  const handleQuickAction = (action) => {
    switch (action) {
      case "newProject":
        alert("Navigate to New Project form")
        break
      case "addWorker":
        alert("Navigate to Add Worker form")
        break
      case "trackExpense":
        alert("Navigate to Track Expense form")
        break
      case "safetyReport":
        alert("Navigate to Safety Report form")
        break
      default:
        break
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: "New Project", icon: Building2, color: "blue", action: "newProject" },
              { label: "Add Worker", icon: Users, color: "green", action: "addWorker" },
              { label: "Track Expense", icon: DollarSign, color: "purple", action: "trackExpense" },
              { label: "Safety Report", icon: AlertTriangle, color: "orange", action: "safetyReport" },
            ].map(({ label, icon: Icon, color, action }) => (
              <button
                key={label}
                onClick={() => handleQuickAction(action)}
                className={`p-4 border-2 border-dashed border-gray-300 rounded-lg text-center hover:border-${color}-500 hover:bg-${color}-50 transition-colors`}
              >

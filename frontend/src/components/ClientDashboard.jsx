...
const ClientDashboard = ({ user, onLogout }) => {
  ...
  const fetchClientProjects = async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/projects/client/${user.id}`, {
        headers: {
  ...
  const searchProjectById = async () => {
    ...
    setError("")
    try {
      const response = await fetch(`/api/projects/${searchId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
      ...
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            ...
            <button onClick={onLogout} className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">
              Logout
            </button>
          </div>
        </div>
        ...
        {selectedProject && (
          <div className="bg-white rounded-lg shadow p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                <h3 className="font-semibold text-gray-700 mb-2">Budget Information</h3>
                <p>
                  <strong>Total Budget:</strong> {formatCurrency(selectedProject.budget)}
                </p>
                <p>
                  <strong>Spent:</strong> {formatCurrency(selectedProject.spent || 0)}
                </p>
                <p>
                  <strong>Remaining:</strong>{" "}
                  {formatCurrency((selectedProject.budget || 0) - (selectedProject.spent || 0))}
                </p>
                <div className="mt-2">
              ...
              <div>
                ...
                <p className="mt-2">
                  <strong>Contractor:</strong> {selectedProject.contractor || "Not assigned"}
                </p>
              </div>
            </div>
